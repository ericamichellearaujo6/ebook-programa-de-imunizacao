...
local function showShieldAndButton(sceneGroup, centerX, centerY)
    ...
    if not escudo then
        escudo = display.newImage(sceneGroup, "assets/gotinhaBebe.png")
        escudo.x = centerX
        escudo.y = centerY - 50
        escudo.xScale = 1.5
        escudo.yScale = 1.5
...
local function recreateAnimation(sceneGroup, centerX, centerY)
    ...
    local imageSheet = graphics.newImageSheet("assets/spriteCrianca.png", sheetOptions)
...
function scene:create(event)
    ...
    local bg = display.newImageRect(sceneGroup, "assets/calendarioCrianca.png", 768, 1024)
    bg.x = centerX
    bg.y = centerY

    -- Carregar o som do botão e da cena
    somBotao = audio.loadSound("assets/som.mp3")
    audioCrianca = audio.loadSound("assets/audioCrianca.mp3")
    ...
    local function onNavegarTap(event)
        composer.gotoScene("calendarioCrianca", { effect = "slideLeft", time = 500 })
    end
    ...
    btSomL = createButton(
        ...
        0.5,
        function()
            if not canalC or not audio.isChannelActive(canalC) then
                canalC = audio.play(audioCrianca)
            end
...
function scene:destroy(event)
    ...
    if audioCrianca then
        audio.dispose(audioCrianca)
    end
end
